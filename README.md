...
The `dbgen` expects `dists.dss` in its working directory. If necessary, the location of the `dists.dss` file can be specified with the `-b` command line argument. CMake is configured to copy the `dists.dss` to the build directory automatically. A typical invocation:
